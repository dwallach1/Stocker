...
class Miner(object):
    ...
    def mine(self):
        ...
        # sys.stdout.flush()
        t = trange(len(self.queries), total=total, unit='query', desc='Configuring',dynamic_ncols=True, leave=True)
        for i in t:
            # os.system('clear')
            q = self.queries[i]
